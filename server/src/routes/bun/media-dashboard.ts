...
export async function mediaCustomRoute(req: Request): Promise<Response> {
  ...
  const upsert = db.transaction(() => {
    ...
    const existingModel = db.prepare("SELECT id, modality, priority, key_id FROM media_models WHERE platform = 'custom' AND model_id = ? LIMIT 1")
    ...
    const priority = existingModel && existingModel.modality === parsed.data.modality
      ...
      : (db.prepare('SELECT COALESCE(MAX(priority), 0) AS maxPriority FROM media_models WHERE modality = ?').get(parsed.data.modality) as { maxPriority: number }).maxPriority + 1;

    if (existingModel) {
      db.prepare("UPDATE media_models SET display_name = ?, modality = ?, priority = ?, enabled = 1, quota_label = ?, key_id = ? WHERE id = ?")
    ...
    const model = db.prepare("INSERT INTO media_models (platform, model_id, display_name, modality, priority, enabled, quota_label, key_id) VALUES ('custom', ?, ?, ?, ?, 1, ?, ?)")
