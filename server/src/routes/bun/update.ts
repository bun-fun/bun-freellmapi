...
// Dashboard update checker (ported from upstream /api/update + the
// /api/settings/update-check opt-in). Bun route style: module-level state
// replaces the router-factory closure.
 ...
 * on the way in rather than after it is already resident.
 */
...
// Read lazily rather than at module load: imports hoist above any bootstrap
// code that might set the kill-switch programmatically (tests), and reading
// process.env per request is free next to a network round trip.
const isUpdateCheckDisabled = (): boolean =>
  env.FREELLMAPI_UPDATE_CHECK?.trim().toLowerCase() === 'off';
...
async function performCheck(currentIdentity: Identity): Promise<CheckResult> {
  ...
  if (response.status === 401 || response.status === 403 || response.status === 429) {
    const atomResponse = await fetch(`https://github.com/${REPOSITORY}/commits/main.atom`, {
      headers: {
...
function check(currentIdentity: Identity): Promise<CheckResult> {
  ...
  const request = performCheck(currentIdentity)
    ...
    .catch((error: unknown) => {
      failedAt = Date.now();
      console.error('[update] GitHub compare check failed', error);
      throw error;
    })
    .finally(() => {
...
function latestRelease(): Promise<LatestRelease> {
  ...
  const request = performReleaseCheck()
    .then((result) => {
      releaseCache = { result, cachedAt: Date.now() };
      releaseFailedAt = null;
      return result;
    })
    .catch((error: unknown) => {
      releaseFailedAt = Date.now();
      console.error('[update] GitHub latest-release check failed', error);
      throw error;
    })
    .finally(() => {
...
/**
 * GET /api/update/release — the latest published release, for the dashboard's
 * automatic reminder (#782). The browser cannot ask GitHub itself — the CSP
 * this server sends is `connect-src 'self'`, so a direct fetch to
 * api.github.com is blocked on every web install. Routing it through here also
 * means the opt-in is enforced where it cannot be bypassed: when the check is
 * off nothing leaves the box, not even a request that is thrown away afterwards.
 */
export async function updateRoute(_req: Request, url: URL): Promise<Response> {
  const path = url.pathname;

  if (path === '/api/update/release') {
    if (isUpdateCheckDisabled() || !isAutoUpdateCheckEnabled()) {
      return jsonResponse({ disabled: true });
    }
    try {
      return jsonResponse(await latestRelease());
    } catch {
      return jsonResponse({
        error: { message: 'Unable to check for the latest release', type: 'upstream_error' },
      }, 502);
    }
  }
  ...
  if (path === '/api/update/check') {
    ...
    try {
      return jsonResponse(await check(currentIdentity));
    } catch {
      return jsonResponse({
