...
describe('Anthropic-compatible /v1/messages', () => {
  ...
  // A pinned /v1/messages request previously preferred exactly ONE catalog row,
  // so when that provider rate-limited, the fallback loop walked the rest of
  // the chain and answered with a DIFFERENT model. Every other surface already
  // routes a pinned id over its unified model group as a strict chain; these
  // tests hold /v1/messages to the same contract.
