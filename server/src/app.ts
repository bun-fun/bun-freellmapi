  ...
  listen(port: number): any {
    ...
    this._bunServer = Bun.serve({
      ...
      async fetch(req: Request) {
        ...
        if (pathname.startsWith('/api/update')) return addCors(req, await updateRoute(req, url));
        if (pathname.startsWith('/api/compression')) return addCors(req, await compressionRoute(req, url));
        if (pathname.startsWith('/api/client-profiles')) return addCors(req, await clientProfilesRoute(req, url));
